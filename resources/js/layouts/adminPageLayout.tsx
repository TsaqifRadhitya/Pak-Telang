...
export default function AdminPageLayout({ children, page }: Props) {
    ...
    return (
        <div className="flex min-h-screen flex-col gap-y-5 lg:gap-y-0 bg-[#EBEFFF] text-[#3B387E] lg:px-10">
            <Head title={page} />
            ...
            {hamburgerMenu && (
                <div className="fixed inset-0 z-50 bg-black/50 lg:hidden" onClick={() => setHamburgerMenu(false)}>
                    <motion.nav
                        ...
                        className="absolute left-0 top-0 flex h-full w-2/3 origin-left flex-col gap-y-5 bg-white px-5 pt-5 sm:w-1/2"
                    >
                        ...
                        <ul className="space-y-4">
                            {['Dashboard', 'Mitra', 'Produk', 'Profile'].map((item) => (
                                <li key={item} className="flex items-center gap-x-1.5 text-xl cursor-pointer" onClick={() => page !== item && router.get(route(`admin.${item.toLowerCase()}`))}>
                                    <img src={`https://ybcvbaxalqwrvgemxdzc.supabase.co/storage/v1/object/public/paktelang/Asset/Icon/${item}.svg`} alt="" />
                                    <h2>{item}</h2>
                                </li>
                            ))}
                        </ul>
                    ...
                    <ul className="flex flex-col gap-y-5">
                        {['Dashboard', 'Mitra', 'Produk', 'Profile'].map((item) => (
                            <li
                                key={item}
                                className={cn(
                                    'flex items-center gap-x-1.5 rounded-l-2xl border border-r-0 border-[#AFB3FF] py-1.5 pl-4 cursor-pointer',
